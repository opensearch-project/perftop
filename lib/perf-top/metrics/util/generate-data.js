...
var env = require("../../../env");
/**
 * Makes a HTTP(S) request to "{endpoint}/_plugins/_performanceanalyzer/metrics?metrics=${metrics}&agg=${aggregates}&dim=${dimensions}&nodes=all"
...
function getMetricData (endpoint, metrics, aggregates, dimensions, done) {
  ...
  var urlOptions = helper.getURLOptions(endpoint, `${env.getPaUrlPrefix()}?${metricParam}${aggParam}${dimParam}&nodes=all`);
...
/**
 * Makes a HTTP(S) request to "{endpoint}/_plugins/_performanceanalyzer/metrics/units"
 * and parses the response to a hashmap object.
 *
...
function getMetricUnits (endpoint, done) {
  var urlOptions = helper.getURLOptions(endpoint, `${env.getPaUrlPrefix()}/units`);
  helper.makeRequest(urlOptions, function (response) {
    if (response === '') {
